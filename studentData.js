// This is the script where all the infomation of hcc studnet exist.
const studentData = [
    // student name
    {
      Name: "Shekhar Kumar ",
      Joining: "07.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Shekhar.png",
      section: "A",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/ShekharKumar.html",
      result_Info: [
        {
          Date: "12.06.2021",
          Marks: "52",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "19.06.2021",
          Marks: "89",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "03.07.2021",
          Marks: "65",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "17.07.2021",
          Marks: "64",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "24.07.2021",
          Marks: "60",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "31.07.2021",
          Marks: "62",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "07.08.2021",
          Marks: "68",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "14.08.2021",
          Marks: "85",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "28.08.2021",
          Marks: "62",
          Progress: "Upward ↑",
          Question_link: "NotFound.html",
        },
        {
          Date: "09.04.2022",
          Marks: "85",
          Progress: "Upward ↑",
          Question_link: "HCCKatra_09_04_2022_Questions_batch_A.html",
        },
        
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Nisha Kumari",
      Joining: "10.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Nisha.png",
      section: "A",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/NishaKumari.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Supriya Kumari",
      Joining: "07.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Supriya.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/SupriyaKumari.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Zeba Praveen",
      Joining: "06.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Zeba.png",
      section: "A",
      Status: "Inactive",
      result_page: "https://charmingprogrammer.github.io/HCC/ZebaPraveen.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Kajal Kumari",
      Joining: "01.07.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Kajal.png",
      section: "A",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Kajal.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Abhinash Kumar",
      Joining: "29.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Abhinash.png",
      section: "A",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Abhinash.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Jyotee Kumari",
      Joining: "05.07.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Jyotee.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Jyotee.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Dhiraj Kumar",
      Joining: "05.07.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Dhiraj.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Dhiraj.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Niraj Kumar",
      Joining: "05.07.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Niraj.png",
      section: "C",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Niraj.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Mistee Kumari",
      Joining: "29.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Mistree.jpg",
      section: "C",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Mistree.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Yash Raj",
      Joining: "29.06.2021",
      Pic: "https://charmingprogrammer.github.io/HCC/Yash.png",
      section: "C",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/YashRaj.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Manisha Kumari",
      Joining: "23.03.2022",
      Pic: "https://charmingprogrammer.github.io/HCC/Manisha.png",
      section: "C",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/YashRaj.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Pritee Kumari",
      Joining: "25.01.2022",
      Pic: "https://i.ibb.co/r607D56/20220404-054033-0000.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Pritee.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Niloo Kumari",
      Joining: "26.12.2021",
      Pic: "https://i.ibb.co/r607D56/20220404-054033-0000.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Niloo.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Nigam Kumar",
      Joining: "24.12.2021",
      Pic: "https://i.ibb.co/5L49LQL/20220405-153726-removebg-preview.png",
      section: "C",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Nigam.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Krishna Kumar",
      Joining: "29.12.2021",
      Pic: "https://i.ibb.co/xM91LQ1/20220405-153705-removebg-preview.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Krishna.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Arushi Kumari",
      Joining: "03.11.2021",
      Pic: "https://i.ibb.co/jT4nJCn/20220405-153752-removebg-preview.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Arushi.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Aradhna Kumari",
      Joining: "03.11.2021",
      Pic: "https://i.ibb.co/nPFChLB/20220405-153807-removebg-preview.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Aradhna.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Gokul Kumar",
      Joining: "02.04.2022",
      Pic: "https://i.ibb.co/D8QtzDz/20220405-153717-removebg-preview.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Gokul.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
    // student name
    {
      Name: "Gautam Kumar",
      Joining: "02.04.2022",
      Pic: "https://i.ibb.co/6FpbXy6/20220405-153657-removebg-preview.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Gautam.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
        // student name
    {
      Name: "Nisha kumari",
      Joining: "04.04.2022",
      Pic: "https://i.ibb.co/wyvkH5Y/20220405-153735-removebg-preview.png",
      section: "B",
      Status: "Active",
      result_page: "https://charmingprogrammer.github.io/HCC/Gautam.html",
      result_Info: [
        {
          Date: "12.09.2020",
          Marks: "36",
          Progress: "Upward ↑",
          Question_link: "https://manoarya.com/Question.html",
        },
        //   add in this week marks
      ],
    },
    // end
  ];
  //  script  for pulling the main student data form studentData.js file
  const students_data = document.getElementById("students_data");
  // loop all the data
  studentData.map((student) => {
    console.log(student);
    students_data.insertAdjacentHTML(
      "beforeend",
      `<a href="${student.result_page}">
  <div class="box2">
    <img src="${student.Pic}" alt="${student.Name}" />
    <h3>${student.Name}</h3>
    <p>
    <b>
      Joining Date : ${student.Joining} <br />
      Section : ${student.section} |
      Status : ${student.Status}</b>
    </p>
  </div>
  </a> `
    );
  });
  